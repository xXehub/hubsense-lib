...
-- Auto Join Functions
local function GetAvailableTable()
	...
	for _, tbl in ipairs(Tables:GetChildren()) do
		if tbl:IsA("Model") then
			...
			-- Check conditions:
			-- 1. Must have player count info
			-- 2. Must have prompt
			-- 3. Must NOT be full
			-- 4. Must be "Waiting" (game not started yet)
			if nbPlayers and prompt and starting then
				...
				if current and max then
					...
					-- Only join if:
					-- - Not full (current < max)
					-- - Game is waiting for players (not started)
					local isWaiting = starting:lower():find("waiting") ~= nil
					local notFull = current < max
					
					if notFull and isWaiting then
						...
						if not notFull then
							print("[Auto Join Debug] Skipped " .. tbl.Name .. " - FULL (" .. nbPlayers .. ")")
						elseif not isWaiting then
							print("[Auto Join Debug] Skipped " .. tbl.Name .. " - Game started (" .. starting .. ")")
						end
					end
				end
			end
	...
	if #availableTables == 0 then
		print("[Auto Join Debug] No available tables found!")
		return nil
	end
	
	-- Sort by player count (prefer tables with more players - game starts sooner)
	...
	-- Random pick from best tables (same player count)
	local chosen = bestTables[math.random(1, #bestTables)]
	
	print("[Auto Join] Selected: " .. chosen.Table.Name .. " (" .. chosen.Players .. ") from " .. #bestTables .. " best tables")
...
local function StartAutoJoin()
	...
	print("[Auto Join] Started scanning for available tables...")
	
	AutoJoinConnection = task.spawn(function()
		while AutoJoinEnabled do
			-- Check if player is already in a game (seated at table or in match)
			local inGame = game.Players.LocalPlayer.PlayerGui:FindFirstChild("InGame")
			if inGame then
				local frame = inGame:FindFirstChild("Frame")
				if frame and frame.Visible then
					IsInGame = true
					print("[Auto Join] Already in game, waiting...")
					task.wait(AutoJoinDelay)
					continue
				end
			end
			
			-- Also check if player is seated (already joined a table)
			local humanoid = game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("Humanoid")
			if humanoid and humanoid.Sit then
				print("[Auto Join] Already seated at table, waiting...")
				task.wait(AutoJoinDelay)
				continue
			end
			...
			if tableInfo then
				-- Use JoinTable which teleports first
				local success = JoinTable(tableInfo)
				if success then
					print("[Auto Join] Successfully joined: " .. tableInfo.Table.Name)
					task.wait(3) -- Wait for game to potentially start
				else
					print("[Auto Join] Failed to join table, retrying...")
					task.wait(1)
				end
			else
				print("[Auto Join] No available tables, scanning...")
			end
