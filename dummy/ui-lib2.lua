...
local Passed, Statement = pcall(function()
	...
	window.Key = Enum.KeyCode.Insert
	
	--
	local wordsuggester = window:CreatePage({Icon = "rbxassetid://8547236654"})
	--
	local infosection = wordsuggester:CreateSection({Name = "Status", Size = 100, Side = "Left"})
	local searchsection = wordsuggester:CreateSection({Name = "Search Words", Size = 380, Side = "Left"})
	local filtersection = wordsuggester:CreateSection({Name = "Filter Settings", Size = 180, Side = "Right"})
	local resultssection = wordsuggester:CreateSection({Name = "Results Display", Size = 300, Side = "Right"})
	
	-- Status Info
	local statusToggle = infosection:CreateToggle({
		Name = "📚 Database Status: Loading...",
		State = false,
		Callback = function(state) end
	})
	
	local wordCountToggle = infosection:CreateToggle({
		Name = "📊 Words Loaded: 0",
		State = false,
		Callback = function(state) end
	})
	
	-- Search Input using Dropdown (hacky way for text input display)
	local searchResults = searchsection:CreateDropdown({
		Name = "🔍 Type to search (use chat or custom input)",
		State = 1,
		Options = {"No results yet", "Type letters to search..."},
		Callback = function(index)
			-- Display only
		end
	})
	
	-- Create search instruction
	searchsection:CreateToggle({
		Name = "💡 How to use: Set filters, type search query",
		State = false,
		Callback = function() end
	})
	
	-- Filter Min Characters
	local minSlider = filtersection:CreateSlider({
		Name = "Min Length",
		State = 1,
		Min = 1,
		Max = 50,
		...
		Callback = function(value)
			minCharacters = math.floor(value)
		end
	})
	
	-- Filter Max Characters
	local maxSlider = filtersection:CreateSlider({
		Name = "Max Length",
		State = 100,
		Min = 1,
		Max = 100,
		...
		Suffix = " chars",
		Callback = function(value)
			maxCharacters = math.floor(value)
		end
	})
	
	-- Apply Filter (using Toggle as button)
	local applyButton = filtersection:CreateToggle({
		Name = "🔄 Click to Apply Filters",
		State = false,
		Callback = function(state)
			if state then
				if minCharacters > maxCharacters then
					maxCharacters = minCharacters
					maxSlider:Set(maxCharacters)
				end
				
				statusToggle.Name = "📚 Database Status: Reloading..."
				ReloadWords()
				...
				spawn(function()
					...
					statusToggle.Name = "📚 Database Status: Ready ✅"
					wordCountToggle.Name = "📊 Words Loaded: " .. #Words
					applyButton:Set(false)
				end)
			end
		end
	})
	...
	-- Result Display Area
	local resultDisplay = resultssection:CreateDropdown({
		Name = "📝 Search Results",
		State = 1,
		Options = {"Results will appear here"},
		Callback = function() end
	})
	
	-- Page Info
	local pageInfo = resultssection:CreateToggle({
		Name = "📄 Page: 0/0 | Total: 0 words",
		State = false,
		Callback = function() end
	})
	
	-- Previous Page
	local prevButton = resultssection:CreateToggle({
		Name = "⬅️ Previous Page",
		State = false,
		Callback = function(state)
			if state and #currentSearchResults > 0 and currentPage > 1 then
				...
				resultDisplay.Options = displayWords
				resultDisplay:Set(1)
				pageInfo.Name = "📄 Page: " .. currentPage .. "/" .. totalPages .. " | Total: " .. #currentSearchResults .. " words"
				
				prevButton:Set(false)
			end
		end
	})
	
	-- Next Page
	local nextButton = resultssection:CreateToggle({
		Name = "➡️ Next Page",
		State = false,
		Callback = function(state)
			if state and #currentSearchResults > 0 then
				local totalPages = math.ceil(#currentSearchResults / wordsPerPage)
	...
	-- Search Function using keybind
	local searchKeybind = searchsection:CreateKeybind({
		Name = "Search Key (Press to search typed query)",
		State = {UserInputType = "KeyCode", Name = "Return"},
		Callback = function(key)
			-- When Enter is pressed, perform search
		end
	})
	
	-- Manual search trigger
	local searchButton = searchsection:CreateToggle({
		Name = "🔍 Click to Search Current Input",
		State = false,
		Callback = function(state)
			if state then
				if not loaded then
					pageInfo.Name = "⏳ Please wait, loading words..."
					searchButton:Set(false)
					return
				end
				
				-- Get search text from user (you need to implement input method)
				local searchText = currentSearchText
				
				if searchText == "" or #searchText < 1 then
					pageInfo.Name = "⚠️ Enter at least 1 character"
					searchButton:Set(false)
					return
				end
				
				currentPage = 1
				currentSearchResults = SuggestWords(searchText, 1000)
				
				if #currentSearchResults == 0 then
					resultDisplay.Options = {"No results found for: '" .. searchText .. "'"}
					resultDisplay:Set(1)
					pageInfo.Name = "❌ No words found"
				else
					local totalPages = math.ceil(#currentSearchResults / wordsPerPage)
					local startIndex = 1
					...
					resultDisplay.Options = displayWords
					resultDisplay:Set(1)
					pageInfo.Name = "📄 Page: 1/" .. totalPages .. " | Total: " .. #currentSearchResults .. " words"
				end
	...
	-- Input text setter (example dropdowns for common searches)
	local quickSearchOptions = {"a", "b", "c", "d", "e", "f", "g", "h", "cat", "dog", "test", "word"}
	local quickSearch = searchsection:CreateDropdown({
		Name = "Quick Search Templates",
		State = 1,
		Options = quickSearchOptions,
		Callback = function(index)
			currentSearchText = quickSearchOptions[index]
		end
	})
	...
	spawn(function()
		while not loaded do
			wait(0.5)
			statusToggle.Name = "📚 Database Status: Loading... (" .. #Words .. ")"
		end
		statusToggle.Name = "📚 Database Status: Ready ✅"
		wordCountToggle.Name = "📊 Words Loaded: " .. #Words .. " words"
	end)
end)
--