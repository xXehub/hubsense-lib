...
-- Type a word into the game using multiple methods
local function TypeWord(word)
	...
	-- Method 1: Find TextBox and set Text directly
	local textBox = GetInputTextBox()
	if textBox then
		print("[Auto Answer] Found TextBox: " .. textBox:GetFullName())
		
		-- Try setting text and firing FocusLost
		pcall(function()
			textBox.Text = word
			-- Fire FocusLost to submit
			textBox:ReleaseFocus(true) -- true = enterPressed
			success = true
			print("[Auto Answer] Method 1 (TextBox.Text + ReleaseFocus) applied")
		end)
		
		if not success then
			pcall(function()
				textBox.Text = word
				-- Fire FocusLost event manually
				if textBox.FocusLost then
					textBox.FocusLost:Fire(true) -- true = enterPressed
				end
				success = true
				print("[Auto Answer] Method 1b (TextBox.Text + Fire FocusLost) applied")
			end)
		end
	end
	
	-- Method 2: Use VirtualInputManager to type keys
	if not success then
		pcall(function()
			local vim = game:GetService("VirtualInputManager")
			
			-- Focus on textbox first if available
			if textBox then
			...
			for i = 1, #word do
				...
				-- Send key event
				vim:SendKeyEvent(true, char, false, game) -- keyDown
				task.wait(0.02)
				vim:SendKeyEvent(false, char, false, game) -- keyUp
				task.wait(0.02)
			end
			...
			success = true
			print("[Auto Answer] Method 2 (VirtualInputManager keypress) applied")
		end)
	end
	
	-- Method 3: Use keypress if available
	if not success then
		pcall(function()
			if keypress and keyrelease then
				...
				success = true
				print("[Auto Answer] Method 3 (keypress/keyrelease) applied")
			end
		end)
	end
	
	-- Method 4: Fire remote event directly if we can find it
	if not success then
		pcall(function()
			local rs = game:GetService("ReplicatedStorage")
			-- Common remote names for word submission
			local remoteNames = {"SubmitWord", "Answer", "SendWord", "WordSubmit", "TypeWord", "Submit"}
			for _, name in ipairs(remoteNames) do
				local remote = rs:FindFirstChild(name, true)
				if remote and remote:IsA("RemoteEvent") then
					remote:FireServer(word)
					success = true
					print("[Auto Answer] Method 4 (RemoteEvent " .. name .. ") fired")
					break
				elseif remote and remote:IsA("RemoteFunction") then
					remote:InvokeServer(word)
					success = true
					print("[Auto Answer] Method 4 (RemoteFunction " .. name .. ") invoked")
					break
				end
			end
		end)
...
local function ProcessAutoAnswer()
	...
	if now - AutoAnswerDebugTimer > 3 then
		...
		print("[Auto Answer Debug] MyTurn=" .. tostring(myTurn) .. ", Letter=" .. tostring(requiredLetter) .. ", Typed=" .. tostring(AutoAnswerTypedThisTurn))
		if not myTurn then
	...
	-- Need to know what letter to start with
	...
	print("[Auto Answer] Required letter: '" .. requiredLetter .. "'")
