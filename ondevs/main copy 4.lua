...
-- Results Groupbox (Right column): proper table with columns
local ResultsBox = Tabs.Main:AddRightGroupbox('Search Results')

-- Table header with columns
ResultsBox:AddLabel('─────────────────────────────────', false)
ResultsBox:AddLabel('No  │  Result', false)
ResultsBox:AddLabel('─────────────────────────────────', false)

-- Create table rows with proper formatting (No | Word)
local ROWS = 15
local RowLabels = {}
for i = 1, ROWS do
...
local function UpdateResultsDisplay()
	...
	-- Fill visible rows with formatted table data (No | Result)
	local row = 1
	for i = startIndex, endIndex do
		if row > ROWS then break end
		local rowNum = string.format('%02d', i)
		RowLabels[row]:SetText(rowNum .. '  │  ' .. currentSearchResults[i])
		row = row + 1
	end
