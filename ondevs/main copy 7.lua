...
local AutoJoinGamemode = "Last Letter" -- Target gamemode
...
local function GetAvailableTable(minPlayers, excludeTableName)
	...
	if not Tables then 
		print("[Auto Join Debug] Tables folder not found!")
		return nil 
	end
	...
	-- Sort by player count (prefer tables with more players)
	table.sort(availableTables, function(a, b)
		return a.CurrentPlayers > b.CurrentPlayers
	end)
	
	-- Get tables with highest player count
	local maxPlayers = availableTables[1].CurrentPlayers
	local bestTables = {}
	for _, t in ipairs(availableTables) do
		if t.CurrentPlayers == maxPlayers then
			table.insert(bestTables, t)
		end
	end
	
	-- Random pick from best tables
	local chosen = bestTables[math.random(1, #bestTables)]
	print("[Auto Join] Selected: " .. chosen.Table.Name .. " (" .. chosen.Players .. ")")
...
local function GetCurrentTableInfo()
	...
	if not IsAtTable() then
		print("[GetCurrentTableInfo] Not at table (PreGame not visible)")
		return nil
	end
	
	-- Since we can't use SeatPart (it's nil), we need to find the table differently
	-- Check which table has the player nearby
	local Tables = workspace:FindFirstChild("Tables")
	if not Tables then 
		print("[GetCurrentTableInfo] Tables folder not found!")
		return nil 
	end
	
	local hrp = game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
	if not hrp then 
		print("[GetCurrentTableInfo] HumanoidRootPart not found!")
		return nil 
	end
	...
	print("[GetCurrentTableInfo] Player position: " .. tostring(hrp.Position))
	
	for _, tbl in ipairs(Tables:GetChildren()) do
		if tbl:IsA("Model") then
			-- Find any BasePart in table to measure distance (try multiple parts)
			local tablePart = nil
			...
			if tablePart then
				local dist = minPartDist
				-- Increased distance threshold to 50 studs
				if dist < closestDist and dist < 50 then
					closestDist = dist
					closestTable = tbl
					print("[GetCurrentTableInfo] Candidate table: " .. tbl.Name .. " at distance " .. string.format("%.1f", dist))
				end
			end
		end
	end
	
	if not closestTable then 
		print("[GetCurrentTableInfo] No table found within 50 studs!")
		-- List all tables and their distances for debug
		for _, tbl in ipairs(Tables:GetChildren()) do
			if tbl:IsA("Model") then
				local tablePart = tbl:FindFirstChildOfClass("BasePart")
				if tablePart then
					local dist = (hrp.Position - tablePart.Position).Magnitude
					print("[GetCurrentTableInfo] Table " .. tbl.Name .. " distance: " .. string.format("%.1f", dist))
				end
			end
		end
		return nil 
	...
	print("[GetCurrentTableInfo] Closest table: " .. closestTable.Name .. " at " .. string.format("%.1f", closestDist) .. " studs")
	
	-- Get table info
	local nbPlayers = nil
	local starting = nil
	
	for _, desc in ipairs(closestTable:GetDescendants()) do
		if desc:IsA("BillboardGui") then
			for _, child in ipairs(desc:GetDescendants()) do
				if child:IsA("TextLabel") then
					if child.Name == "NbOfPlayers" then
						nbPlayers = child.Text
						print("[GetCurrentTableInfo] Found NbOfPlayers: " .. nbPlayers)
					elseif child.Name == "Starting" then
						starting = child.Text
						print("[GetCurrentTableInfo] Found Starting: " .. starting)
					end
				end
			end
		end
	...
	if nbPlayers then
		local current, max = nbPlayers:match("(%d+)/(%d+)")
		if current and max then
			print("[GetCurrentTableInfo] Parsed: " .. current .. "/" .. max)
			return {
				Table = closestTable,
				Players = nbPlayers,
				CurrentPlayers = tonumber(current),
				MaxPlayers = tonumber(max),
				Status = starting or ""
			}
		else
			print("[GetCurrentTableInfo] Failed to parse player count from: " .. nbPlayers)
		end
	else
		print("[GetCurrentTableInfo] NbOfPlayers label not found in table!")
		-- Debug: list all TextLabels in table
		for _, desc in ipairs(closestTable:GetDescendants()) do
			if desc:IsA("TextLabel") then
				print("[GetCurrentTableInfo] Found TextLabel: " .. desc.Name .. " = '" .. desc.Text .. "'")
			end
		end
	end
...
local function LeaveTable()
	print("[Auto Join] Attempting to leave table...")
	
	local playerGui = game.Players.LocalPlayer.PlayerGui
	local preGame = playerGui:FindFirstChild("PreGame")
	
	if not preGame then
		print("[Auto Join] PreGame GUI not found - not at table?")
		return false
	end
	
	local frame = preGame:FindFirstChild("Frame")
	if not frame then
		print("[Auto Join] PreGame.Frame not found!")
		return false
	end
	
	if not frame.Visible then
		print("[Auto Join] PreGame.Frame not visible - not at table?")
		return false
	end
	
	local leaveBtn = frame:FindFirstChild("Leave")
	if not leaveBtn then
		print("[Auto Join] Leave button not found in PreGame.Frame!")
		-- Try searching deeper
		for _, desc in ipairs(frame:GetDescendants()) do
			if desc:IsA("TextButton") and (desc.Name == "Leave" or desc.Text:lower():find("leave")) then
				leaveBtn = desc
				print("[Auto Join] Found Leave button at: " .. desc:GetFullName())
				break
			end
		end
	end
	...
	if not leaveBtn then
		print("[Auto Join] Could not find Leave button anywhere!")
		return false
	end
	
	print("[Auto Join] Found Leave button: " .. leaveBtn:GetFullName())
	print("[Auto Join] Button Visible: " .. tostring(leaveBtn.Visible))
	print("[Auto Join] Button Text: " .. tostring(leaveBtn.Text))
	
	local clicked = false
	
	-- Method 1: getconnections + Fire (MOST RELIABLE for exploits)
	if not clicked then
		local ok, err = pcall(function()
			if getconnections then
				local connections = getconnections(leaveBtn.MouseButton1Click)
				print("[Auto Join] MouseButton1Click connections: " .. tostring(#connections))
				...
				print("[Auto Join] getconnections not available")
			end
		end)
		if not ok then
			print("[Auto Join] getconnections MouseButton1Click error: " .. tostring(err))
		elseif clicked then 
			print("[Auto Join] SUCCESS: getconnections MouseButton1Click")
		end
	end
	
	-- Method 2: getconnections Activated
	if not clicked then
		local ok, err = pcall(function()
			if getconnections then
				local connections = getconnections(leaveBtn.Activated)
				print("[Auto Join] Activated connections: " .. tostring(#connections))
				if #connections > 0 then
					for _, conn in pairs(connections) do
						conn:Fire()
					end
		...
		if not ok then
			print("[Auto Join] getconnections Activated error: " .. tostring(err))
		elseif clicked then 
			print("[Auto Join] SUCCESS: getconnections Activated")
		end
	end
	
	-- Method 3: firesignal MouseButton1Click
	if not clicked then
		local ok, err = pcall(function()
			if firesignal then
				firesignal(leaveBtn, "MouseButton1Click")
				clicked = true
			else
				print("[Auto Join] firesignal not available")
			end
		end)
		if not ok then
			print("[Auto Join] firesignal MouseButton1Click error: " .. tostring(err))
		elseif clicked then 
			print("[Auto Join] SUCCESS: firesignal MouseButton1Click")
		end
	end
	
	-- Method 4: firesignal Activated
	if not clicked then
		local ok, err = pcall(function()
			if firesignal then
				firesignal(leaveBtn, "Activated")
				clicked = true
			else
				print("[Auto Join] firesignal not available")
			end
		...
		if not ok then
			print("[Auto Join] firesignal Activated error: " .. tostring(err))
		elseif clicked then 
			print("[Auto Join] SUCCESS: firesignal Activated")
		end
	end
	
	-- Method 5: fireclick (some executors)
	if not clicked then
		local ok, err = pcall(function()
			if fireclick then
				fireclick(leaveBtn)
				...
				print("[Auto Join] fireclick not available")
			end
		end)
		if not ok then
			print("[Auto Join] fireclick error: " .. tostring(err))
		elseif clicked then 
			print("[Auto Join] SUCCESS: fireclick")
		end
	end
	
	-- Method 6: VirtualInputManager (synthesize mouse click)
	if not clicked then
		local ok, err = pcall(function()
			local VirtualInputManager = game:GetService("VirtualInputManager")
			local absPos = leaveBtn.AbsolutePosition
			local absSize = leaveBtn.AbsoluteSize
			local centerX = absPos.X + (absSize.X / 2)
			local centerY = absPos.Y + (absSize.Y / 2)
			
			print("[Auto Join] VIM click at: " .. tostring(centerX) .. ", " .. tostring(centerY))
		...
		if not ok then
			print("[Auto Join] VirtualInputManager error: " .. tostring(err))
		elseif clicked then 
			print("[Auto Join] SUCCESS: VirtualInputManager")
		end
	end
	
	-- Method 7: mouse1click (cursor simulation)
	if not clicked then
		local ok, err = pcall(function()
			if mouse1click then
				local absPos = leaveBtn.AbsolutePosition
				...
				print("[Auto Join] mouse1click not available")
			end
		end)
		if not ok then
			print("[Auto Join] mouse1click error: " .. tostring(err))
		elseif clicked then 
			print("[Auto Join] SUCCESS: mouse1click")
		end
	end
	
	-- Method 8: Direct MouseButton1Click event fire (alternative)
	if not clicked then
		local ok, err = pcall(function()
			-- Try to directly fire the event
			leaveBtn.MouseButton1Click:Fire()
			clicked = true
		end)
		if not ok then
			print("[Auto Join] MouseButton1Click:Fire() error: " .. tostring(err))
		elseif clicked then 
			print("[Auto Join] SUCCESS: MouseButton1Click:Fire()")
		end
	end
	
	-- Method 9: Try using syn mouse functions (Synapse specific)
	if not clicked then
		local ok, err = pcall(function()
			if syn and syn.mouse1click then
				...
				if syn.mouse_move then
					syn.mouse_move(centerX, centerY)
					task.wait(0.05)
				end
				syn.mouse1click()
				clicked = true
			else
				print("[Auto Join] syn not available")
			end
		end)
		if not ok then
			print("[Auto Join] syn.mouse1click error: " .. tostring(err))
		elseif clicked then 
			print("[Auto Join] SUCCESS: syn.mouse1click")
		end
	end
	
	-- If no method worked, print summary
	if not clicked then
		print("[Auto Join] All click methods failed! Button may need different approach.")
	end
	
	-- Wait and verify if we actually left
	task.wait(0.5)
	
	local stillAtTable = IsAtTable()
	if stillAtTable then
		print("[Auto Join] WARNING: Still at table after click attempts!")
		-- Even if clicked=true, we might still be at table
		return false
	else
		print("[Auto Join] Successfully left table!")
		return true
	end
end

local function TeleportToTable(tableInfo)
	local player = game.Players.LocalPlayer
	local char = player.Character
	if not char then return false end
	
	-- Don't teleport if already at a table (use PreGame GUI check)
	if IsAtTable() then
		print("[Auto Join] Already at table, skipping teleport")
		return false
	end
	...
	-- Find position to teleport to
	local targetPart = tableInfo.SeatPart
	if not targetPart then
		-- Try to find any BasePart in the table
		targetPart = tableInfo.Table:FindFirstChildOfClass("BasePart")
	end
	
	if not targetPart then
		-- Use prompt's parent
		local promptParent = tableInfo.Prompt.Parent
		while promptParent and not promptParent:IsA("BasePart") do
	...
	if targetPart and targetPart:IsA("BasePart") then
		-- Teleport near the table (slightly above to avoid collision)
		local targetCFrame = targetPart.CFrame * CFrame.new(0, 3, -2)
		hrp.CFrame = targetCFrame
		print("[Auto Join] Teleported to: " .. tableInfo.Table.Name)
		return true
	end
	
	print("[Auto Join] Could not find teleport target!")
	return false
end
...
local function TriggerProximityPrompt(prompt)
	...
	if not prompt or not prompt:IsA("ProximityPrompt") then
		print("[Auto Join Debug] Invalid prompt!")
		return false
	end
	
	print("[Auto Join Debug] Triggering prompt: " .. prompt:GetFullName())
	
	-- Method 1: fireproximityprompt (most executors support this)
	if fireproximityprompt then
		print("[Auto Join Debug] Using fireproximityprompt")
		fireproximityprompt(prompt)
		return true
	end
	
	-- Method 2: Direct fire (some games/executors)
	local success = pcall(function()
		prompt:Fire()
	end)
	if success then
		print("[Auto Join Debug] Used prompt:Fire()")
		return true
	end
	
	-- Method 3: InputHoldBegin/End (requires being near)
	local ok = pcall(function()
		prompt:InputHoldBegin()
		task.wait(math.max(prompt.HoldDuration, 0) + 0.2)
		prompt:InputHoldEnd()
	end)
	
	if ok then
		print("[Auto Join Debug] Triggered via InputHold")
		return true
	end
	
	print("[Auto Join Debug] All trigger methods failed!")
	return false
end

local function JoinTable(tableInfo)
	if not tableInfo then return false end
	
	-- Check if already at a table (use PreGame GUI check)
	if IsAtTable() then
		print("[Auto Join] Already at table, skipping")
		return false
	end
	
	print("[Auto Join] Joining table: " .. tableInfo.Table.Name .. " (" .. tableInfo.Players .. ")")
	
	-- Step 1: Teleport to the table
	local teleported = TeleportToTable(tableInfo)
	if not teleported then
		print("[Auto Join] Teleport failed, trying to trigger anyway...")
	end
	
	-- Step 2: Wait a moment for teleport to complete
	task.wait(0.3)
	
	-- Step 3: Trigger the proximity prompt
	local success = TriggerProximityPrompt(tableInfo.Prompt)
...
local function StartAutoJoin()
	...
	AutoJoinConnection = task.spawn(function()
		while AutoJoinEnabled do
			-- Use IsAtTable() which checks PreGame GUI visibility
			local isAtTable = IsAtTable()
			
			-- Check if player is in active game
			local inGame = game.Players.LocalPlayer.PlayerGui:FindFirstChild("InGame")
			local inActiveGame = false
			if inGame then
			...
			if inActiveGame then
				-- In active game, just wait
				print("[Auto Join] In game, waiting...")
				task.wait(AutoJoinDelay)
			elseif isAtTable then
				-- Player is at a table (PreGame visible)
				local currentTable = GetCurrentTableInfo()
				
				if currentTable then
					print("[Auto Join] At table: " .. currentTable.Table.Name .. " (" .. currentTable.Players .. ") - Status: " .. tostring(currentTable.Status))
					...
					if currentTable.CurrentPlayers <= 1 then
						...
						print("[Auto Join] Table has " .. currentTable.CurrentPlayers .. " players, staying...")
						task.wait(AutoJoinDelay)
					end
				else
				...
				if tableInfo then
					print("[Auto Join] Found: " .. tableInfo.Table.Name .. " (" .. tableInfo.Players .. ")")
					local success = JoinTable(tableInfo)
					if success then
						print("[Auto Join] Joined!")
						task.wait(2)
					else
						print("[Auto Join] Failed to join, retrying...")
						task.wait(1)
					end
				else
					print("[Auto Join] No tables with 1+ players, waiting...")
					task.wait(AutoJoinDelay)
				end
			end
		end
...
local function StopAutoJoin()
	...
	if AutoJoinConnection then
		task.cancel(AutoJoinConnection)
...
-- Track last choices to detect turn change
local AutoAnswerLastChoices = ""

-- Get current word from InGame GUI (last letter needed)
local function GetCurrentGameWord()
	...
	if currentWordContainer then
		...
		-- Check Case container first (from debug: CurrentWord > Case > Letter)
		...
		-- Fallback: check numbered frames
...
local function IsMyTurn()
	...
	if not choicesFrame then return false, "no Choices frame" end
	
	-- Count visible choice buttons (don't require Choices.Visible)
	...
	if visibleButtons == 0 then return false, "no visible buttons (" .. #choicesFrame:GetChildren() .. " children)" end
...
-- Get available letter choices from Choices frame
local function GetLetterChoices()
	...
	if choicesFrame then
		-- Don't check choicesFrame.Visible - just check individual buttons
		for _, child in ipairs(choicesFrame:GetChildren()) do
			if child:IsA("TextButton") and child.Visible and child.Text ~= "" then
				table.insert(choices, {
					Button = child,
					Letter = child.Text:upper(),
					Name = child.Name
				})
			end
		end
...
-- Find best word starting with a letter from our word database
local function FindBestWord(startLetter)
	if not loaded then 
	...
	local wordList = WordDictionary[startLetter]
	
	if not wordList then 
		print("[Auto Answer] No wordList for letter '" .. startLetter .. "'")
		return nil 
	end
	
	if #wordList == 0 then 
		print("[Auto Answer] wordList empty for letter '" .. startLetter .. "'")
		return nil 
	end
	...
	if #validWords == 0 then
		-- Fallback: use any word if no words in range
		return wordList[math.random(1, math.min(100, #wordList))]
	end
...
-- Count words available for each letter choice
local function CountWordsForLetter(letter)
	if not loaded then return 0 end
	if not WordDictionary then return 0 end
	
	-- IMPORTANT: Convert to lowercase for dictionary lookup
	letter = letter:lower()
	local wordList = WordDictionary[letter]
	
	if not wordList then return 0 end
	
	-- Count words in valid length range
	local count = 0
	for _, word in ipairs(wordList) do
		local len = #word
		if len >= AutoAnswerMinLength and len <= AutoAnswerMaxLength then
...
-- Click a letter choice button
local function ClickLetterChoice(choice)
	if not choice or not choice.Button then 
		print("[Auto Answer] No button to click!")
		return false 
	end
	
	local button = choice.Button
	print("[Auto Answer] Attempting to click: " .. button:GetFullName())
	
	local success = false
	
	-- Method 1: getconnections MouseButton1Click (MOST RELIABLE for UI)
	pcall(function()
		if getconnections then
			...
			if #conns > 0 then
				for _, conn in ipairs(conns) do
					if conn.Function then
						conn:Fire()
						success = true
						print("[Auto Answer] Method 1 (getconnections Fire) SUCCESS")
						break
					end
				end
			end
	...
	-- Method 2: getconnections with Function call
	pcall(function()
		if getconnections then
			local conns = getconnections(button.MouseButton1Click)
			for _, conn in ipairs(conns) do
				if conn.Function then
					conn.Function()
					success = true
					print("[Auto Answer] Method 2 (getconnections Function) SUCCESS")
					break
				end
			end
		end
	...
	pcall(function()
		if firesignal then
			...
			success = true
			print("[Auto Answer] Method 3 (firesignal Click) SUCCESS")
		end
	end)
	if success then return true end
	...
	pcall(function()
		if firesignal then
			...
			success = true
			print("[Auto Answer] Method 4 (firesignal Activated) SUCCESS")
		end
	end)
	if success then return true end
	...
	pcall(function()
		if fireclick then
			fireclick(button)
			success = true
			print("[Auto Answer] Method 5 (fireclick) SUCCESS")
		end
	end)
	if success then return true end
	
	-- Method 6: Simulate full click sequence
	pcall(function()
		if firesignal then
			...
			firesignal(button, "MouseButton1Click")
			firesignal(button, "Activated")
			firesignal(button, "MouseLeave")
			success = true
			print("[Auto Answer] Method 6 (full sequence) SUCCESS")
		end
	end)
	if success then return true end
	
	-- Method 7: VirtualInputManager with GuiService offset
	pcall(function()
		...
		success = true
		print("[Auto Answer] Method 7 (VIM+GuiInset) SUCCESS")
	end)
	if success then return true end
	
	-- Method 8: Direct TextButton event fire
	pcall(function()
		button.MouseButton1Click:Fire()
		success = true
		print("[Auto Answer] Method 8 (Direct Fire) SUCCESS")
	end)
	if success then return true end
	
	-- Method 9: Synapse mouse
	pcall(function()
		if syn and syn.mouse1click then
			local guiService = game:GetService("GuiService")
			local guiInset = guiService:GetGuiInset()
			...
			syn.mouse1click()
			success = true
			print("[Auto Answer] Method 9 (syn.mouse) SUCCESS")
		end
	end)
	if success then return true end
	
	-- Method 10: mouse1click global
	pcall(function()
		if mouse1click then
			local guiService = game:GetService("GuiService")
			local guiInset = guiService:GetGuiInset()
			...
			mouse1click()
			success = true
			print("[Auto Answer] Method 10 (mouse1click) SUCCESS")
		end
	end)
	
	if not success then
		print("[Auto Answer] All click methods failed!")
	end
	
	return success
end

-- Build choices key for comparison
local function GetChoicesKey(choices)
	local letters = {}
	for _, c in ipairs(choices) do
		table.insert(letters, c.Letter)
	end
...
local function ProcessAutoAnswer()
	...
	-- Get choices regardless of turn status (for debug)
	local choices = GetLetterChoices()
	
	-- Debug: Print status every 3 seconds
	local now = tick()
	if now - AutoAnswerDebugTimer > 3 then
		AutoAnswerDebugTimer = now
		print("[Auto Answer Debug] IsMyTurn=" .. tostring(myTurn) .. " (" .. tostring(reason) .. "), Choices=" .. #choices)
	end
	
	-- Not our turn
	if not myTurn then 
		if AutoAnswerLastChoices ~= "" then
			print("[Auto Answer] Turn ended (" .. tostring(reason) .. "), resetting...")
		end
		AutoAnswerLastChoices = ""
		return 
	end
	...
	if #choices == 0 then 
		print("[Auto Answer] My turn but no choices found!")
		return 
	end
	
	-- Build unique key from current choices
	local choicesKey = GetChoicesKey(choices)
	
	-- Avoid processing same choices twice
	if choicesKey == AutoAnswerLastChoices and AutoAnswerLastChoices ~= "" then 
		return 
	end
	
	-- Get current word (for display only)
	local currentWord = GetCurrentGameWord() or ""
	
	print("[Auto Answer] === MY TURN! ===")
	print("[Auto Answer] Word: '" .. currentWord .. "'")
	print("[Auto Answer] Choices: " .. choicesKey .. " (prev: '" .. AutoAnswerLastChoices .. "')")
	
	-- Find best letter choice (most words available)
	local bestChoice = nil
	local bestCount = 0
	
	for _, choice in ipairs(choices) do
		local count = CountWordsForLetter(choice.Letter)
		print("[Auto Answer]   '" .. choice.Letter .. "' = " .. count .. " words")
	...
	AutoAnswerLastChoices = choicesKey
	
	if bestChoice and bestCount > 0 then
		local exampleWord = FindBestWord(bestChoice.Letter)
		print("[Auto Answer] >> Selecting '" .. bestChoice.Letter .. "' (" .. bestCount .. " words, e.g. '" .. (exampleWord or "?") .. "')")
		
		-- Wait delay before clicking
		task.wait(AutoAnswerDelay)
		
		-- Click the choice
		local success = ClickLetterChoice(bestChoice)
		print("[Auto Answer] Click result: " .. tostring(success))
	else
		-- Fallback: random choice
		local randomChoice = choices[math.random(1, #choices)]
		print("[Auto Answer] >> Random pick: '" .. randomChoice.Letter .. "'")
		
		task.wait(AutoAnswerDelay)
		ClickLetterChoice(randomChoice)
	end
	
	print("[Auto Answer] === WAITING FOR NEXT TURN ===")
	
	-- Delay after clicking to let game process
	task.wait(1.5)
end
...
local function StartAutoAnswer()
	...
	AutoAnswerConnection = task.spawn(function()
		while AutoAnswerEnabled do
			pcall(ProcessAutoAnswer) -- Wrap in pcall to prevent crashes
			task.wait(0.3) -- Check every 300ms
		end
...
GameFeaturesBox:AddToggle('AutoJoin', {
	...
	Callback = function(Value)
		...
		if Value then
			AutoJoinStatusLabel:SetText('Auto Join: Scanning...')
			StartAutoJoin()
		else
			AutoJoinStatusLabel:SetText('Auto Join: Disabled')
			StopAutoJoin()
		end
	end
})
...
GameFeaturesBox:AddButton({
	...
	Func = function()
		...
		if not Tables then
			...
			print('[Scan] ERROR: workspace.Tables not found!')
			return
		end
		...
		for _, tbl in ipairs(Tables:GetChildren()) do
			if tbl:IsA("Model") and count < 3 then
				count = count + 1
				print('[Scan] Table: ' .. tbl.Name)
				for _, desc in ipairs(tbl:GetDescendants()) do
					if desc:IsA("BillboardGui") then
						print('  - BillboardGui: ' .. desc.Name)
					elseif desc:IsA("ProximityPrompt") then
						print('  - ProximityPrompt: ' .. desc.Name .. ' Action: ' .. desc.ActionText)
					elseif desc:IsA("TextLabel") and (desc.Name == "NbOfPlayers" or desc.Name == "Starting") then
						print('  - ' .. desc.Name .. ' = "' .. desc.Text .. '"')
					end
				end
			end
		end
		
		local tableInfo = GetAvailableTable()
		if tableInfo then
			AutoJoinStatusLabel:SetText('Found: ' .. tableInfo.Table.Name .. ' (' .. tableInfo.Players .. ')')
			print('[Scan] Available table: ' .. tableInfo.Table.Name .. ' - ' .. tableInfo.Status)
		else
			AutoJoinStatusLabel:SetText('No available tables found')
			print('[Scan] No waiting tables found')
		end
	end
})
...
GameFeaturesBox:AddButton({
	...
	Func = function()
		local tableInfo = GetAvailableTable()
		if tableInfo then
			AutoJoinStatusLabel:SetText('Teleporting to: ' .. tableInfo.Table.Name .. '...')
			print('[Join] Teleporting and joining: ' .. tableInfo.Table.Name .. ' (' .. tableInfo.Players .. ')')
			
			local success = JoinTable(tableInfo)
			if success then
				AutoJoinStatusLabel:SetText('Joined: ' .. tableInfo.Table.Name .. ' (' .. tableInfo.Players .. ')')
				print('[Join] Successfully joined!')
			else
				AutoJoinStatusLabel:SetText('Failed to join!')
				print('[Join] Failed to trigger proximity prompt')
			end
		else
