	...
	if WaitingForResult and CurrentAnswerWord then
		...
		if currentWord and LastSeenWord and currentWord ~= LastSeenWord then
			...
			if isPartialMatch then
				...
				if RetryCount < MaxRetries then
					...
					-- Use LastSeenWord (original) as prefix
					if LastSeenWord then
						CurrentPrefix = LastSeenWord
					end
		...
		if timeSinceAnswer > 1.5 then
			...
			if RetryCount < MaxRetries then
				...
				ClearInput()
				task.wait(0.2) -- Tunggu sebentar untuk memastikan input bersih
				
				-- STEP 2: Clear input lagi untuk memastikan benar-benar bersih
				ClearInput()
				task.wait(0.1)
				
				-- STEP 3: Gunakan prefix dari LastSeenWord (original word), BUKAN dari word terakhir
				-- Ini penting karena LastSeenWord adalah word yang pertama kali kita lihat di turn ini
				local retryPrefix = nil
				if LastSeenWord and #LastSeenWord > 0 then
					retryPrefix = LastSeenWord:upper()
					print("[Auto Answer] Using original word as prefix: '" .. retryPrefix .. "'")
				elseif CurrentPrefix then
					retryPrefix = CurrentPrefix
					print("[Auto Answer] Fallback to saved prefix: '" .. retryPrefix .. "'")
				end
				
				-- Update CurrentPrefix to use for retry
				if retryPrefix then
