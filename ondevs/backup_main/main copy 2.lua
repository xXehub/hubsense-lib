...
-- Build a table-like list: multiple visible rows (no dropdown)
local ROWS = 15
local RowButtons = {}
for i = 1, ROWS do
	RowButtons[i] = ResultsBox:AddButton({
		Text = '—',
		Func = function()
			local startIndex = (currentPage - 1) * wordsPerPage + 1
			local idx = startIndex + (i - 1)
			local val = currentSearchResults[idx]
			if val then
				-- You can extend this to copy/show more info
				print('Selected:', val)
			end
		end
	})
...
local function UpdateResultsDisplay()
	...
	for i = startIndex, endIndex do
		...
		RowButtons[row]:SetText(currentSearchResults[i])
		row = row + 1
	end
